...
- **graphql**: Upgraded `webonyx/graphql-php` 15.31.4 → 15.32.3 — patches three DoS vulnerabilities:
  - Medium-severity quadratic complexity in `OverlappingFieldsCanBeMerged` — a crafted query could trigger excessive CPU usage ([GHSA-68jq-c3rv-pcrr](https://github.com/advisories/GHSA-68jq-c3rv-pcrr)); fixed in `>=15.31.5`
  - High-severity quadratic validation cost via inline fragment spreads in `OverlappingFieldsCanBeMerged` — O(n²) validation time before the query complexity limit is applied ([GHSA-fc86-6rv6-2jpm](https://github.com/advisories/GHSA-fc86-6rv6-2jpm)); fixed in `>=15.32.2`
  - High-severity unbounded recursion in the parser — a crafted deeply nested input causes a stack overflow and kills the PHP-FPM worker ([GHSA-r7cg-qjjm-xhqq](https://github.com/advisories/GHSA-r7cg-qjjm-xhqq)); fixed in `>=15.32.3`
